...
{
    public class TransferToBlobFunction
    {
        private readonly ILogger<TransferToBlobFunction> _logger;
        private IGraphApiClient _graphApiClient;
        private IBlobService _blobService;


        public TransferToBlobFunction(ILogger<TransferToBlobFunction> log, IGraphApiClient graphApiClient, IBlobService blobService)
        {
            _logger = log;
        ...
        public async Task Work_Function([ActivityTrigger] FileData fileInfo)
        {
            ...
            var url = fileInfo.fileUrl.Split('?')[0];

            var output = await getFile(fileInfo.fileUrl);
            await _blobService.UploadBlobFromStream(GetFilenameFromUrl(url),output);
        }

        [FunctionName("TransferToBlobFunctionFunction")]
        ...
        public async Task<Stream> getFile(string fileUrl)
        {
